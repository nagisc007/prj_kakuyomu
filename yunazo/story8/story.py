...
def ep_intro(ma: Master,
    ...
    return ma.story("魔王を崇める村",
            ...
            ma.scene("嫌われの勇者",
                ...
                child.hate(yusha),
                child.throw(yusha, "石"),
                child.know(yusha, "勇者である").set_flag(yusha.name),
                yusha.wonder(child, "自分を知っていたこと"),
                ),
            ma.scene("勇者好きの少年",
                ordy.angry(child),
                ordy.talk(child, "勇者が本当の英雄なんだぞ"),
                ordy.talk(child, "特別な子").ps().set_flag(ordy.name),
                yusha.meet(ordy),
                ordy.know(yusha, "勇者"),
...
def ep_maou(ma: Master,
    ...
    return ma.story("魔王と勇者",
            ma.scene("魔王に救われた村",
                ...
                ordy.talk(sword, "父親の形見").set_flag(sword.name),
                yusha.think(ordy, "貧しい"),
                ordy.talk(yusha, "勇者"),
                yusha.hear(ordy, "祭り"),
                ordy.know(fes),
                ordy.want(yusha),
                yusha.ask(ordy, "自分を勇者と知っている理由"),
                crades.talk(mark).set_flag(mark.name),
                ordy.reply(yusha, "{}から聞いた".format(sailor.name)).set_deflag(yusha.name),
                ),
            ma.scene("三年前の話",
                yusha.teach(ordy, "勇者"),
                ordy.teach(pday),
                ordy.talk(pday, "村が津波で壊滅状態に"),
                ordy.teach(dad, "その津波で亡くなる").set_flag(tale.name),
                yusha.know(dad, "三年前に亡くなる"),
                ordy.talk(pday, "村を襲う勇者を魔王が退治した"),
                ordy.hear(dad, "魔王が本当は勇者だと"),
                yusha.know(vila, "魔王が救った"),
                ),
            ma.scene("裏切りの夜",
                yusha.eat(),
                ordy.sleep(),
                mam.talk(pday, "以来この村は魔王軍から襲われない"),
                yusha.sleep(),
                mam.whisper(yusha, "ごめんなさい"),
                ),
            )
...
def ep_festa(ma: Master,
    ...
    return ma.story("魔王の謎",
            ma.scene("捕まる勇者",
                ...
                mam.talk(yusha, "勇者に夫は殺された").set_deflag(tale.name),
                people.speak(info="勇者たちを殺せ"),
                ),
            ma.scene("少年の勇気",
                ordy.rescue(yusha),
                ordy.help(panna, "勇者の仲間は助けた"),
                panna.talk(crades, "自分が村人から勇者を助け出す"),
                ordy.tell("魔王はそんなこと望んでない"),
                ordy.talk(people, "本当のこと"),
            ...
            ma.scene("魔王の真実",
                ...
                ordy.talk(people, "魔王が自分は昔勇者だったと語った"),
                ordy.teach(info="魔王はかつて勇者だった"),
                ordy.talk(sword, "本当は魔王の持ち物だった").set_deflag(sword.name),
